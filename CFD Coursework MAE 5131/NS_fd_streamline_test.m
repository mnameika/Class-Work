...
[xFine, yFine] = meshgrid(0:0.01:1, 0:0.01:1);
...
pFine = interp2(xPlot,yPlot, pHold', xFine, yFine, 'cubic');



contourf(xFine, yFine, pFine, -1:0.001:1, 'LineStyle', 'none')
hold on
quiver(xPlot(1:3:end,1:3:end),yPlot(1:3:end,1:3:end),u0(1:3:end,1:3:end)',v0(1:3:end,1:3:end)', 2, '-k', 'linewidth', 0.5)

xlabel('$x$', 'fontsize', 35, 'interpreter', 'latex')
ylabel('$y$', 'fontsize', 35, 'interpreter', 'latex')
axis([0 1 0 1])
cb = colorbar();
colormap('Turbo')
ylabel(cb,'Pressure $P$', 'fontsize', 35, 'interpreter', 'latex')
% streamline(xPlot,yPlot,u0',v0', 0.9388,0.69388)
strmln = stream2(xPlot,yPlot,u0',v0', 0.7755, 0.7755);

strmPts = 2000;

strmDbl = strmln{1};

plot(strmDbl(1:strmPts,1), strmDbl(1:strmPts,2), 'k-', 'linewidth', 1.1)
